...
export default {
  setup() {
    ...
    const filterRecentSixMonths = () => {
      return afterSalesProtectionRecords.value.filter(record => {
    ...
    // 初始化时筛选最近六个月的记录
    const afterSalesProtectionRecords = ref([]);
    onMounted(() => {
      fetchOrders();
    });
    ...
    const fetchOrders = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:8080/api/orders', {
          params: { 
            username,
            status: 'afterSales' // 修改为 'afterSales'
          } 
        }); // 替换为你的后端API地址
        console.log('Orders fetched successfully:', response.data);
        afterSalesProtectionRecords.value = response.data.filter(record => record.status === 'afterSales'); // 前端再次过滤确保只留下status为afterSales的数据
        console.log('afterSalesProtection records:', afterSalesProtectionRecords.value);
        filteredRecords.value = filterRecentSixMonths();
        console.log('Filtered records:', filteredRecords.value);
        if (filteredRecords.value.length === 0) {
          console.warn('No afterSalesProtection records found');
        }
